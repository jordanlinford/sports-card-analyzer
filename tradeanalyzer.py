...
def analyze_trade(card1_data, card2_data):
    """
    Analyze a potential trade between two cards
    
    Args:
        card1_data (pd.Series): Data for the first card (user's card)
        card2_data (pd.Series): Data for the second card (target card)
        
    Returns:
        dict: Analysis results including recommendation and detailed metrics
    """
    try:
        # Extract values
        card1_value = float(card1_data['Market Price'])
        card2_value = float(card2_data['Market Price'])
        
        # Calculate value difference percentage
        value_diff_pct = ((card2_value - card1_value) / card1_value) * 100
        
        # Determine if it's a fair trade (within 10% value difference)
        fair_trade = abs(value_diff_pct) <= 10
        
        # Calculate ROI potentials
        card1_roi = ((card1_value - float(card1_data['Purchase Price'])) / float(card1_data['Purchase Price'])) * 100
        card2_roi = ((card2_value - float(card2_data['Purchase Price'])) / float(card2_data['Purchase Price'])) * 100
        
        # Compare volatilities
        card1_volatility = float(card1_data['Volatility'])
        card2_volatility = float(card2_data['Volatility'])
        volatility_diff = card2_volatility - card1_volatility
        
        # Get trend scores
        card1_trend = str(card1_data.get('Trend Score', 'N/A'))
        card2_trend = str(card2_data.get('Trend Score', 'N/A'))
        
        # Generate risk assessment
        if volatility_diff > 5:
            risk_assessment = f"Their card is more volatile ({card2_volatility:.1f}% vs {card1_volatility:.1f}%)"
        elif volatility_diff < -5:
            risk_assessment = f"Your card is more volatile ({card1_volatility:.1f}% vs {card2_volatility:.1f}%)"
        else:
            risk_assessment = "Both cards have similar volatility levels"
            
        # Generate future outlook
        try:
            card1_trend_value = float(card1_trend.rstrip('%')) if card1_trend != 'N/A' else 0
            card2_trend_value = float(card2_trend.rstrip('%')) if card2_trend != 'N/A' else 0
            trend_diff = card2_trend_value - card1_trend_value
            
            if abs(trend_diff) < 5:
                future_outlook = "Both cards show similar market trends"
            else:
                better_trend = "their card" if trend_diff > 0 else "your card"
                future_outlook = f"Market momentum favors {better_trend}"
        except ValueError:
            future_outlook = "Insufficient trend data for detailed comparison"
            
        # Generate recommendation
        if not fair_trade:
            if value_diff_pct > 0:
                recommendation = f"You're receiving ${value_diff_pct:.1f}% more value"
                if card2_volatility > card1_volatility:
                    recommendation += ", but with higher market volatility"
            else:
                recommendation = f"You're giving up ${abs(value_diff_pct):.1f}% value"
                if card2_roi > card1_roi:
                    recommendation += ", but the target card has higher ROI potential"
        else:
            if card2_roi > card1_roi:
                recommendation = "Fair value trade with higher ROI potential in their card"
            elif card2_roi < card1_roi:
                recommendation = "Fair value trade but your card has higher ROI potential"
            else:
                recommendation = "Fair value trade with similar ROI potential"
                
        # Add data confidence assessment for new cards
        recent_sales_count2 = card2_data.get('Recent Sales Count', 0)
        if recent_sales_count2 == 0:
            recommendation += "\n⚠️ Limited market data available for their card. Consider additional research."
        elif recent_sales_count2 < 5:
            recommendation += "\n⚠️ Limited recent sales data. Market analysis may not be comprehensive."
...
def display_trade_analyzer():
    """Display the trade analyzer interface"""
    st.header("🔄 Trade Analyzer")
    
    try:
        # Load collection for your card selection
        try:
            collection_df = pd.read_csv('card_collection.csv')
            if collection_df.empty:
        ...
        # Create main columns for data entry
        col1, col2 = st.columns(2)
        
        # Prepare card options from your collection
        ...
        with col1:
            ...
            if your_card:
                ...
                # Display your card details
                st.markdown(f"""
                **Current Value:** ${float(your_card_data['Market Price']):,.2f}
                **Purchase Price:** ${float(your_card_data['Purchase Price']):,.2f}
                **Condition:** {your_card_data.get('Condition', 'N/A')}
                """)

        # Their Card Column
        with col2:
            st.subheader("Their Card")
            their_player = st.text_input("Player Name", key="their_player")
            their_card_desc = st.text_input("Card Description (e.g., 2018 Panini Prizm #280)", key="their_card_desc")
            their_condition = st.selectbox(
                "Condition",
                ["Raw", "PSA 9", "PSA 10"],
                key="their_condition"
            )
            their_asking_price = st.number_input(
                "Asking Price ($)",
                min_value=0.0,
                value=0.0,
                step=1.0,
        ...
        with col2:
            analyze_button = st.button("Analyze Trade", key="analyze_trade_button", use_container_width=True)

        if analyze_button:
            if not your_card or not their_player or not their_card_desc:
                st.warning("Please provide all required information for both cards")
                return
            ...
            try:
                with st.spinner("Analyzing trade..."):
                    ...
                    # Fetch market data for their card
                    progress_bar = st.progress(0)
                    st.info("Gathering market data for the trade card...")
                    
                    search_query = f"{their_player} {their_card_desc}"
                    progress_bar.progress(25)
                    
                    their_sales = get_ebay_sales(search_query, their_condition)
                    progress_bar.progress(50)
                    
                    # Calculate metrics for their card
                    volatility, trend_score = calculate_market_metrics(their_sales)
                    progress_bar.progress(75)
                    
                    # Calculate market statistics
                    market_stats = {
                        'avg_price': their_sales['Price'].mean() if not their_sales.empty else their_asking_price,
                        'max_price': their_sales['Price'].max() if not their_sales.empty else their_asking_price,
                        'min_price': their_sales['Price'].min() if not their_sales.empty else their_asking_price,
                        'recent_sales_count': len(their_sales) if not their_sales.empty else 0
                    }
                    
                    their_card_data = pd.Series({
                        'Player': their_player,
                        'Card Description': their_card_desc,
                        'Condition': their_condition,
                        'Purchase Price': their_asking_price,
                        'Market Price': market_stats['avg_price'],
                        'Volatility': volatility,
                        'Trend Score': trend_score,
                        'Recent Sales Count': market_stats['recent_sales_count'],
                        'Max Price': market_stats['max_price'],
                        'Min Price': market_stats['min_price']
                    })
                    ...
                    if analysis:
                        ...
                        with comp_col1:
                            ...
                            st.markdown(f"""
                            **Volatility:** {analysis['detailed_metrics']['card1']['volatility']:.1f}%
                            **Recent Sales:** {your_card_data.get('Recent Sales Count', 'N/A')}
                            """)

                        # Their Card Analysis
                        with comp_col2:
                            st.markdown("### Their Card")
                            st.metric(
                                "Market Value",
                            ...
                            st.markdown(f"""
                            **Volatility:** {analysis['detailed_metrics']['card2']['volatility']:.1f}%
                            **Recent Sales:** {their_card_data.get('Recent Sales Count', 'N/A')}
                            """)

                        # Trade Recommendation
                        st.markdown("## Recommendation")
                        recommendation_color = "green" if "Accept" in analysis['recommendation'] else "red"
                        st.markdown(f"""
                        <div style='padding: 20px; border-radius: 5px; background-color: {"rgba(0,255,0,0.1)" if recommendation_color == "green" else "rgba(255,0,0,0.1)"}'>
                            <h3 style='color: {recommendation_color}; margin-top: 0;'>{analysis['recommendation']}</h3>
                            <p><strong>Risk Assessment:</strong> {analysis['risk_assessment']}</p>
                            <p><strong>Future Outlook:</strong> {analysis['future_outlook']}</p>
                        </div>
                        """, unsafe_allow_html=True)

                        # Market Data for Trade Card
                        if not their_sales.empty:
                            with st.expander("📊 Trade Card Market Data"):
                                st.markdown(f"""
                                **Recent Sales Data ({their_condition}):**
                                - Average Price: ${market_stats['avg_price']:.2f}
                                - Highest Sale: ${market_stats['max_price']:.2f}
                                - Lowest Sale: ${market_stats['min_price']:.2f}
                                - Number of Sales: {market_stats['recent_sales_count']}
                                """)
                                
                                # Plot recent sales
                                fig = px.scatter(their_sales, x='Date', y='Price',
                                               title=f'Recent Sales - {their_player} {their_card_desc}')
                                st.plotly_chart(fig, use_container_width=True)
                        else:
                            st.warning("""
                            No recent sales data available for the trade card.
                            Analysis is based on the provided asking price.
                            Consider researching more about this card's market value.
                            """)
                    else:
